...
class ProjectWay < ActiveRecord::Base
  ...
  belongs_to :project
  has_many :way_nodes, :class_name => "ProjectWayNode", :foreign_key => "way_id", :primary_key => "osm_id", :order => 'sequence_id', :conditions => proc {"project_way_nodes.project_id = #{project_id}"}
  has_many :nodes, :through => :way_nodes, :order => 'project_way_nodes.sequence_id'
  ...
  def self.intersects(l)
    joins("left join project_way_nodes wn on project_ways.osm_id = wn.way_id and project_ways.project_id = wn.project_id").
      ...
      having("st_intersects(st_makeline(n.geom), ?::geometry)", l).
