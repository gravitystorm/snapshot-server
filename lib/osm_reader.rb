    ...
    while @parser.read do
      ...
      if @parser.node_type == XML::Reader::TYPE_END_ELEMENT
        ...
        when "relation"
          ...
          @current_entity = ProjectRelation.new(project_id: @project.id, osm_id: osm_id, version: version, user_id: user_id,
                                           tstamp: tstamp, changeset_id: changeset_id)
        ...
        when "member"
          raise unless @current_entity.is_a?(ProjectRelation)
          sequence_id = @current_entity.relation_members.length # start from 0
          member_osm_id = @parser["ref"]
          member_type = @parser["type"][0].upcase
          member_role = @parser["role"]
          rm = ProjectRelationMember.create!(project_id: @project.id, relation_id: @current_entity.osm_id, member_id: member_osm_id,
                                             member_type: member_type, member_role: member_role, sequence_id: sequence_id)
          @current_entity.relation_members << rm
